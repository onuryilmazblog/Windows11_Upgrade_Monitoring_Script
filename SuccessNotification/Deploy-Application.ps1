...
function Write-Logger() {
    ...
    End {
        ...
        Write-Logger -Message "SuccessNotification: script başlatıldı."
        ...
        $SuccessFolderPath = "$scriptPath\SuccessNotification"


        ## Get Task Function
        ## Example: GetTask "Win11_Upgrade"
        Function GetTask ($GetTaskName) {
            [int32]$GetTaskCount = (Get-ScheduledTask -TaskPath "\" -TaskName "$GetTaskName" -ErrorAction SilentlyContinue | Select-Object -ExpandProperty TaskName -ErrorAction SilentlyContinue).Count
            return [int32]$GetTaskCount
        }
        ...
        if ((GetTask $UpgradeTaskName) -eq 0) {
            Write-Log -Message "SuccessNotification: $UpgradeTaskName taskı başarıyla kaldırıldı."      
            ## New Task
            ## Task Scheduler Control
            if ((GetTask $RemoveTaskName) -eq 0) {
                ...
                $Action= New-ScheduledTaskAction -Execute "$envWinDir\system32\wscript.exe" -Argument "$SuccessFolderPath\Remove_Task.vbs"
                Register-ScheduledTask -TaskName "$RemoveTaskName" -User $User -Action $Action -Trigger $Trigger -Settings $Set –Force
                Start-Sleep 10
                Write-Log -Message "SuccessNotification: $RemoveTaskName adında oluşturuldu."
            }
            else {
                Write-Log -Level Warn -Message "SuccessNotification: $RemoveTaskName adında task bulundu."
            ...
            Start-Sleep 10
            if ((GetTask $UpgradeTaskName) -gt 0) {
                Write-Logger -Level Error -Message "SuccessNotification: $UpgradeTaskName taskı kaldırılamadı."
