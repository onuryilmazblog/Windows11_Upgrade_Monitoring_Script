...
Try {
	...
	[string]$appVendor = ''
	[string]$appName = 'Upgrade Status Monitoring'
	[string]$appVersion = ''
	[string]$appArch = ''
	[string]$appLang = 'EN'
...
function Write-Logger() {
    ...
    End {
        ...
        Write-Logger -Message "UpgradeStatusMonitoring: Script başlatıldı."
        ...
        Function UpgradeTrigger {
            ...
            Write-Logger -Message "Win11 Upgrade Software Center'da trigger edildi."
        }

        ## Add Upgrade Reg Key
        ## Example: Add-UpgradeReg "UpgradeTaskStatus" "1"
        ...
        Write-Logger -Message "UpgradeStatusMonitoring: 5 dakika beklemeye alındı."
        Start-Sleep 300
        $EvaluationState = (FeatureUpdateCheck).EvaluationState
        
        if ($EvaluationState -ne 7) {
            Write-Logger -Message "UpgradeStatusMonitoring: EvaluationState değeri 7 olarak algılanmadığı için beklemeye alındı."
            [int32]$WaitingInstallCount = 0
            While (($EvaluationState -ne 7) -and $WaitingInstallCount -lt 36)
            {
                Start-Sleep 300
                $WaitingInstallCount++
                $EvaluationState = (FeatureUpdateCheck).EvaluationState
                if ($EvaluationState -eq 8) { $WaitingInstallCount = 37; Write-Logger -Level Warn -Message "UpgradeStatusMonitoring: EvaluationState değeri 8 olarak algılandı." }
                if ($EvaluationState -eq 13) { $WaitingInstallCount = 37; Write-Logger -Level Warn -Message "UpgradeStatusMonitoring: EvaluationState değeri 13 olarak algılandı." }
            }
            
            $EvaluationState = (FeatureUpdateCheck).EvaluationState
            if ($EvaluationState -eq 13) {
                $FeatureErrorCode = (FeatureUpdateCheck).ErrorCode
                Write-Logger -Level Error -Message "Error Code: $FeatureErrorCode."
                Add-UpgradeReg "Upgrade_ErrorCode" "$FeatureErrorCode"
                Start-Sleep 600
                UpgradeTrigger
                Start-Sleep 300
                if ((FeatureUpdateCheck).EvaluationState -eq 13) { Write-Logger -Message "Upgrade_Task: Tetikleme sonrası 10 dakika daha beklemeye alındı."; Start-Sleep 600; UpgradeTrigger }
                if ((FeatureUpdateCheck).EvaluationState -eq 13) { Write-Logger -Message "Upgrade_Task: Tetikleme sonrası 10 dakika daha beklemeye alındı."; Start-Sleep 600 }
                if ((FeatureUpdateCheck).EvaluationState -eq 13) { [System.Environment]::Exit(13) }
            }

            if ($EvaluationState -eq 8) {
                Write-Logger -Message "UpgradeStatusMonitoring: Upgrade_Task scripti yeniden tetiklendi."
                Start-ScheduledTask -TaskName "Win11_Upgrade" -ErrorAction SilentlyContinue
            ...
            Write-Logger -Message "UpgradeStatusMonitoring: EvaluationState değeri olarak $EvaluationState algılandı."
        }
