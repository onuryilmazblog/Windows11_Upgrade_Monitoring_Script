...
Try {
	...
	[string]$appVendor = ''
	[string]$appName = 'AC Adapter Notification'
	[string]$appVersion = ''
	[string]$appArch = ''
	[string]$appLang = 'EN'
...
function Write-Logger() {
    ...
    End {
        ...
        Write-Logger -Message "ACAdaptorNotification: script başlatıldı."

        $TestIsLaptop = (Test-Battery -PassThru).IsLaptop
        if ($TestIsLaptop -eq $True) {    
            Write-Logger -Message "ACAdaptorNotification: Bilgisayar tipi Laptop olarak algılandığı için adaptör kontrolü başlatıldı."
            $TestBattery = Test-Battery
            if (!($TestBattery))
            { 
                Write-Logger -Message "ACAdaptorNotification: Takılı güç adaptörü algılanmadı."
                Show-InstallationRestartPrompt -NoCountdown
            }
            else {
                Write-Logger -Message "ACAdaptorNotification: Takılı güç adaptörü algılandı."
            }
        }
        else {
            Write-Logger -Message "ACAdaptorNotification: Bilgisayar tipi Laptop olarak algılanmadığı için adaptör kontrolü yapılmadı."
        }
